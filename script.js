...
function unlockProducers(producers, coffeeCount) {
  producers.forEach((producers) => {
    if (producers.price / 2 <= coffeeCount) {
...
function getUnlockedProducers(data) {
  ...
  data.producers.forEach((datum) => {
    if (datum.unlocked === true) {
      arr.push(datum);
    }
  });
...
function makeDisplayNameFromId(id) {
  ...
  let splitedStrArr = str.split("_");
  let a = [];

  for (let i = 0; i < splitedStrArr.length; i++) {
    a.push(splitedStrArr[i][0].toUpperCase() + splitedStrArr[i].slice(1));
  }
...
function canAffordProducer(data, producerId) {
  let obj = getProducerById(data, producerId);

  console.log(obj.price);

  if (data.coffee >= obj.price) {
    return true;
  } else {
    return false;
  }
